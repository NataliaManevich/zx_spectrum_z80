...
: _width(width), _height(height)
{
	_window = std::shared_ptr<SDL_Window>(
			SDL_CreateWindow( "Ёмул€тор ZX-Spectrum", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_SHOWN),
			SDL_DestroyWindow);
	if (_window == nullptr)
		throw std::runtime_error(
				std::string("ќшибка оздани€ окна: ") +
				SDL_GetError());
	...
	if (_renderer == nullptr)
		throw std::runtime_error(
				std::string("ќшибка оздани€ рендера: ") +
				SDL_GetError());
}
